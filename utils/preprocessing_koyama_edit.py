...
class Preprocessing_python2(object):
    ...
    def patch_extract(self, data_dir, label_data_dir, prefix = "", image_size = 256, crop_size = 33, stride = 5):
        ...
        reconstruction_path = "%s/data/test_dataset/reconstruction_%stest_dataset_crop%s/" % (memCNN_home, prefix, crop_size)

        label_training_path = "%s/data/training_dataset/label_%straining_dataset_crop%s/" %(memCNN_home, prefix,crop_size)
        label_test_path  = "%s/data/test_dataset/label_%stest_dataset_crop%s/" % (memCNN_home, prefix,crop_size)                
        label_reconstruction_path = "%s/data/test_dataset/label_reconstruction_%stest_dataset_crop%s/" % (memCNN_home, prefix, crop_size)        
        ...
        if os.path.exists(test_path) != True:
            ...
            # 既にdatasetが存在すれば警告
            overwrite_test= input("The filepath\n" + test_path + "\n already exists. Ovewrite? type 'yes' or 'no' with  'sign. \n" )
                ...
                for w in xrange(int((image_size - crop_size) / stride)):
                    ...
                    if (ans_0_number >= ans_1_number and ans == 1) or (ans_0_number <= ans_1_number and ans == 0):
                        ...
                        if file_index <= 80:
                            ...
                            label_training_filenameindex = "label_%straining_image_%03d%03d%03d.tif" %(prefix, file_index, h, w)
                            ...
                            label_test_filenameindex = "label_%stest_image_%03d%03d%03d.tif" %(prefix, file_index, h, w)
